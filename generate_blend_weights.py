...
class GPUStreamProcessor(RealTimeStreamProcessor):
    ...
    def _test_gpu_load(self) -> Dict[str, float]:
        """Test GPU load during processing to verify utilization."""
        gpu_load_data = {
            'before_load': 0.0,
            'during_load': 0.0,
            'after_load': 0.0,
        ...
        try:
            # Get baseline GPU load
            baseline_info = self._get_gpu_info()
            gpu_load_data['before_load'] = baseline_info['load']
            
            # Create multiple large test images for intensive workload
            test_images = []
            for i in range(8):  # More images for heavier load
                test_images.append(np.random.randint(0, 255, (2048, 2048, 3), dtype=np.uint8))  # Larger images
            
            # Perform INTENSIVE GPU operations
            for i in range(len(test_images)):
                gpu_test_image = cv2.UMat(test_images[i])
                
                # HEAVY GPU OPERATIONS TO MAXIMIZE UTILIZATION
                for _ in range(5):  # Multiple rounds
                    # Large kernel operations
                    gpu_test_image = cv2.GaussianBlur(gpu_test_image, (31, 31), 10.0)
                    gpu_test_image = cv2.bilateralFilter(gpu_test_image, 15, 100, 100)
                    
                    # Heavy morphological operations
                    large_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (21, 21))
                    gpu_test_image = cv2.morphologyEx(gpu_test_image, cv2.MORPH_CLOSE, large_kernel)
                    gpu_test_image = cv2.dilate(gpu_test_image, large_kernel, iterations=5)
                    
                    # Resize operations to create computational load
                    h, w = gpu_test_image.get().shape[:2]
                    gpu_test_image = cv2.resize(gpu_test_image, (w*2, h*2), interpolation=cv2.INTER_CUBIC)
                    gpu_test_image = cv2.resize(gpu_test_image, (w, h), interpolation=cv2.INTER_CUBIC)
                    
                    # Color space conversions
                    gpu_hsv = cv2.cvtColor(gpu_test_image, cv2.COLOR_BGR2HSV)
                    gpu_lab = cv2.cvtColor(gpu_test_image, cv2.COLOR_BGR2LAB)
                    gpu_test_image = cv2.cvtColor(gpu_hsv, cv2.COLOR_HSV2BGR)
                    gpu_test_image = cv2.cvtColor(gpu_lab, cv2.COLOR_LAB2BGR)
                    
                    # Heavy filtering
                    gpu_test_image = cv2.fastNlMeansDenoising(gpu_test_image, None, 10, 7, 21)
                    
                    # Multiple edge detection
                    gpu_gray = cv2.cvtColor(gpu_test_image, cv2.COLOR_BGR2GRAY)
                    edges = cv2.Canny(gpu_gray, 50, 150)
                    edges_colored = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)
                    edges_colored = cv2.UMat(edges_colored)
                    gpu_test_image = cv2.addWeighted(gpu_test_image, 0.7, edges_colored, 0.3, 0)
                
                # Check GPU load during intensive processing
                during_info = self._get_gpu_info()
                current_load = during_info['load']
                gpu_load_data['during_load'] = max(gpu_load_data['during_load'], current_load)
                print(f"   Processing image {i+1}: GPU Load: {current_load:.1f}%")
            
            # Get final GPU load
            final_info = self._get_gpu_info()
            gpu_load_data['after_load'] = final_info['load']
            gpu_load_data['max_load'] = max(gpu_load_data['during_load'], gpu_load_data['after_load'])
            
            # Convert all images back to ensure GPU operations completed
            for i in range(len(test_images)):
                _ = cv2.UMat(test_images[i]).get()
            
            print(f"🔬 Intensive GPU Load Test Results:")
            print(f"   Before: {gpu_load_data['before_load']:.1f}%")
            print(f"   During: {gpu_load_data['during_load']:.1f}%")
            print(f"   After: {gpu_load_data['after_load']:.1f}%")
            print(f"   Max: {gpu_load_data['max_load']:.1f}%")
            
        except Exception as e:
            print(f"⚠️  GPU load test failed: {e}")
        
        return gpu_load_data
...
• Automatic fallback to CPU when OpenCL is not beneficial
    ...
    parser.add_argument('--gpu-stress-test', action='store_true', help='Run GPU stress test to verify utilization')
    
    return parser.parse_args()

def run_gpu_stress_test() -> None:
    """Run comprehensive GPU stress test to verify utilization."""
    print("🔥 GPU Stress Test - Verifying GPU Utilization")
    print("=" * 60)
    
    if not OPENCL_GPU_AVAILABLE:
        print("❌ OpenCL not available - cannot run GPU stress test")
        return
    
    try:
        # Initialize GPU processor for testing
        from surround_view import SystemConfig
        config = SystemConfig()
        
        # Create test processor
        processor = GPUStreamProcessor(target_fps=10, max_buffer_size=10)
        
        # Get initial GPU info
        initial_gpu_info = processor._get_gpu_info()
        print(f"📊 Initial GPU Status:")
        print(f"   GPU: {initial_gpu_info['name']}")
        print(f"   Load: {initial_gpu_info['load']:.1f}%")
        print(f"   Memory: {initial_gpu_info['memory_percent']:.1f}%")
        print(f"   Temperature: {initial_gpu_info['temperature']:.0f}°C")
        print()
        
        # Test 1: Basic GPU operations
        print("🧪 Test 1: Basic GPU Operations")
        gpu_load_test = processor._test_gpu_load()
        print()
        
        # Test 2: Intensive workload
        print("🧪 Test 2: Intensive GPU Workload")
        test_images = []
        for i in range(4):
            test_images.append(np.random.randint(0, 255, (1600, 1200, 3), dtype=np.uint8))
        
        import time
        start_time = time.time()
        
        # Process images with intensive GPU operations
        for i, test_img in enumerate(test_images):
            enhanced_img = processor._enhance_gpu_utilization(test_img)
            
            # Monitor GPU during processing
            gpu_info = processor._get_gpu_info()
            print(f"   Image {i+1}: GPU Load: {gpu_info['load']:.1f}%")
        
        end_time = time.time()
        processing_time = (end_time - start_time) * 1000
        
        # Get final GPU info
        final_gpu_info = processor._get_gpu_info()
        print(f"   Processing Time: {processing_time:.1f}ms")
        print(f"   Final GPU Load: {final_gpu_info['load']:.1f}%")
        print()
        
        # Test 3: Continuous stress test
        print("🧪 Test 3: Continuous Stress Test (10 seconds)")
        stress_start = time.time()
        max_load = 0.0
        avg_load = 0.0
        load_samples = []
        
        while time.time() - stress_start < 10:
            # Create larger random workload for intensive processing
            stress_image = np.random.randint(0, 255, (2048, 2048, 3), dtype=np.uint8)
            
            # Run intensive GPU operations
            gpu_stress = cv2.UMat(stress_image)
            
            # Multiple heavy operations to maximize GPU load
            for _ in range(3):
                gpu_stress = cv2.GaussianBlur(gpu_stress, (25, 25), 8.0)
                gpu_stress = cv2.bilateralFilter(gpu_stress, 15, 100, 100)
                
                # Heavy morphological operations
                large_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (17, 17))
                gpu_stress = cv2.morphologyEx(gpu_stress, cv2.MORPH_CLOSE, large_kernel)
                gpu_stress = cv2.dilate(gpu_stress, large_kernel, iterations=3)
                
                # Resize operations
                h, w = gpu_stress.get().shape[:2]
                gpu_stress = cv2.resize(gpu_stress, (w//2, h//2), interpolation=cv2.INTER_CUBIC)
                gpu_stress = cv2.resize(gpu_stress, (w, h), interpolation=cv2.INTER_CUBIC)
            
            # Convert back to ensure operations complete
            _ = gpu_stress.get()
            
            # Monitor GPU load
            gpu_info = processor._get_gpu_info()
            current_load = gpu_info['load']
            load_samples.append(current_load)
            max_load = max(max_load, current_load)
            
            time.sleep(0.05)  # Smaller delay for more intensive testing
        
        avg_load = np.mean(load_samples) if load_samples else 0.0
        
        print(f"   Duration: 10 seconds")
        print(f"   Max GPU Load: {max_load:.1f}%")
        print(f"   Average GPU Load: {avg_load:.1f}%")
        print(f"   Samples: {len(load_samples)}")
        print()
        
        # Final assessment
        print("🎯 GPU Utilization Assessment:")
        if max_load > 60:
            print("   ✅ EXCELLENT - GPU is heavily utilized")
        elif max_load > 40:
            print("   ✅ GOOD - GPU is moderately utilized")
        elif max_load > 20:
            print("   ⚠️  MODERATE - GPU is lightly utilized")
        elif max_load > 10:
            print("   ❌ POOR - GPU shows minimal utilization")
        else:
            print("   ❌ CRITICAL - GPU is barely used")
            print("   💡 Consider:")
            print("      • Checking GPU drivers")
            print("      • Verifying OpenCL installation")
            print("      • GPU may not be properly configured")
            print("      • Try using CUDA instead of OpenCL")
        
        print()
        print("📈 GPU Performance Summary:")
        print(f"   Initial Load: {initial_gpu_info['load']:.1f}%")
        print(f"   Peak Load: {max_load:.1f}%")
        print(f"   Average Load: {avg_load:.1f}%")
        print(f"   Final Load: {final_gpu_info['load']:.1f}%")
...
def main() -> int:
    ...
    # Run GPU stress test if requested
    if args.gpu_stress_test:
        run_gpu_stress_test()
        return 0
